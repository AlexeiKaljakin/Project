...
public class StudentTable extends AbsTable {
    ...
    public void selectAllStudent() throws SQLException {
        String sqlQuery = "SELECT student.id, student.fio, group.name, curator.fio from `group`, student, curator where student.id_group = group.id and group.id_curator = curator.id;";
    ...
    public void selectStudentGroup() throws SQLException {
        String sqlQuery = "select student.fio from student where student.id_group = (select id from `group` where name = 'Сопромат');";
        ResultSet rs = db.executeRequestWithAnswer(sqlQuery);
    ...
    public void insert(Student student){
        final String sqlRequest = String.format("insert into %s (id, fio, sex, id_group) VALUES (%d, '%s', '%s', '%d');",
